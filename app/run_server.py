...
def predict():
	...
	if flask.request.method == "POST":
		...
		sulphates, free_sulfur_dioxide, total_sulfur_dioxide, pH = 0,0,0,0
		request_json = flask.request.get_json()
		if request_json["sulphates"]:
			sulphates = request_json['sulphates']

		if request_json["free_sulfur_dioxide"]:
			free_sulfur_dioxide = request_json['free_sulfur_dioxide']

		if request_json["total_sulfur_dioxide"]:
		...
		logger.info(f'{dt} Data: '
					...
					f'pH = {pH} ')
		try:
			preds = model.predict_proba(pd.DataFrame({"sulphate": [sulphates],
